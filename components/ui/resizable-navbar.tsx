    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-[rgb(180,200,255)] dark:text-[rgb(160,120,255)]"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-[rgba(30,130,255,0.1)] dark:bg-[rgba(160,120,255,0.08)]
"
            />
          )}
          <span className="relative z-20 text-gray-300">{item.name}</span>
        </a>
      ))}
    </motion.div>
  );
